		...
		.controller(
				...
				[
						...
						function($rootScope, $scope, $translate, $http,
								$location, $routeParams, $modal, $popover,
								$timeout, appResourceRoot, ResourceService) {
							...
							// Initialize model
							$scope.model = {
								// Store the main model id, this points to the
									...
									url : url
										...
										.success(
												function(data, status, headers,
													...
													$scope.model.bpmn20DownloadUrl = ACTIVITI.CONFIG.contextRoot
															+ '/app/rest/models/'
															+ $routeParams.modelId
															+ ($routeParams.modelHistoryId == undefined ? ''
																	: '/history/'
																			+ $routeParams.modelHistoryId)
															+ '/bpmn20?version='
															+ Date.now();
													...
													$timeout(function() {
														...
														var viewerUrl = appResourceRoot
																+ "../display/displaymodel.html?version="
																+ Date.now();

														// If Activiti has been
														// deployed inside an
														// AMD environment
														// Raphael will fail to
														// register
														// itself globally until
														// displaymodel.js
														// (which depends ona
														// global Raphale
														// variable) is runned,
														// therefor remove AMD's
														// define method until
														// we have loaded in
														// Raphael and
														// displaymodel.js
							...
							$scope.showVersion = function(version) {
								if (version) {
									if (version.latestVersion) {
										...
										// Show latest version, no
										// history-suffix needed in URL
										$location.path("/processes/"
												+ $scope.model.latestModelId
												+ "/history/" + version.id);
									}
							...
							$scope.duplicateProcess = function() {
								var modalInstance = _internalCreateModal(
										{
											template : 'views/popup/process-duplicate.html?version='
													+ Date.now()
										}, $modal, $scope);
							...
							$scope.toggleHistory = function($event) {
								if (!$scope.historyState) {
									...
									state.popover.$scope.$on('tooltip.hide',
											destroy);
